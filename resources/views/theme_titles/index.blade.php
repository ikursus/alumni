...
<table class="table">
    ...
    <tbody>
    @foreach ($senaraiTitles as $item)

        <tr>
            <td>{{ $item->id }}</td>
            <td>{{ $item->name }}</td>
            <td>
            <a href="{{ route('titles.show', $item->id) }}" class="btn btn-primary btn-sm">VIEW</a>
            <a href="{{ route('titles.edit', $item->id) }}" class="btn btn-warning btn-sm">EDIT</a>
...
<div class="modal fade" id="modal-delete-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <form method="POST" action="{{ route('titles.destroy', $item->id) }}">
    @csrf
    @method('DELETE')
